({
    onInit : function(component, event, helper) {
        var oAuthInitStartLoading = component.getEvent('actionEvent');
        oAuthInitStartLoading.setParams({
            action: 'oAuthInitStartLoading'
        });
        oAuthInitStartLoading.fire();
        ...
        getOAuthStartParams.setCallback(this, function(response) {
            if (response.getState() === 'SUCCESS') {
                ...
                if (parsedResponse.isSuccess) {
                    ...
                    var oAuthInitDoneLoading = component.getEvent('actionEvent');
                    oAuthInitDoneLoading.setParams({
                        action: 'oAuthInitDoneLoading',
                        params: JSON.stringify({
                            'isAuthorized': parsedResponse.isAuthorized,
                            'authorizingUser': parsedResponse.authorizingUser
                        })
                    });
                    oAuthInitDoneLoading.fire();
                }
                else {
