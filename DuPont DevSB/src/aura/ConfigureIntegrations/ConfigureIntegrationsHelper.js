({
    ...
    getAvailableIntegrations : function(component) {
        ...
        var allIntegrations = ['Box', 'Drawloop API', 'DocuSign', 'e-SignLive', 'Lob', 'Office 365', 'SMTP', 'FTP', 'SFTP', 'Nintex Workflow Cloud'];
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === 'SUCCESS') {
                    ...
                    if (integrations !== null && integrations.length > 0) {
                        var type = integrations[0].Loop__Type__c;
    ...
    updateValues : function(component, values) {
        ...
        var protocolDomainWithoutPrefix;
        if (type === 'FTP' || type === 'SFTP') {
            var integrationProtocolDomain = integrationData.Loop__Base_URL__c;
            var pattern = new RegExp('^(f.*?)://|(s.*?)://');
            if (pattern.test(integrationProtocolDomain)) {
                var prefix = pattern.exec(integrationProtocolDomain);
                component.set('v.isSecure', prefix[0] === 'ftps://');
                component.set('v.protocol', prefix[1]);
                protocolDomainWithoutPrefix = integrationProtocolDomain.substring(prefix[0].length, integrationProtocolDomain.length);
            }
        }
        ...
        if (protocolDomain && integrationData.Loop__Base_URL__c) protocolDomain.set('v.value', protocolDomainWithoutPrefix);
    ...
    getValues : function(component) {
        ...
        if (type === 'FTP' || type === 'SFTP') {
            protocolDomain = component.get('v.protocol') + '://' + protocolDomain.get('v.value');
        }
        ...
        var values = {
            ...
            baseUrl: protocolDomain ? protocolDomain : baseUrl ? baseUrl.get('v.value') : '',
    ...
    oAuthOffice365 : function(component) {
        ...
        var getO365OAuthUrl = component.get('c.getO365OAuthUrl');
        getO365OAuthUrl.setParams({
            nonLightningSessionId: component.get('v.sessionId'),
            baseUrl: component.find('baseUrl').get('v.value'),
            domain: component.get('v.loopUrl')
        });
        getO365OAuthUrl.setCallback(this, function(response) {
            if (response.getState() === 'SUCCESS') {
                var parsedResponse = JSON.parse(response.getReturnValue());
                if (parsedResponse.isSuccess) {
        ...
        $A.enqueueAction(getO365OAuthUrl);
    },
    upsertO365Integration : function(component) {
         ...
         upsertO365Integration.setCallback(this, function(response) {
             if (response.getState() === 'SUCCESS') {
                 ...
                 else if (parsedResponse.errorType === 'duplicateName') {
                     this.addErrorToField(component.find('name'), 'Name must be unique');
                     component.set('v.errors',['These following fields have errors: Name']);
                     this.fireErrorEvent(component, parsedResponse.errorMessage);
                 }
                 else {
                     this.fireErrorEvent(component, parsedResponse.errorMessage);
